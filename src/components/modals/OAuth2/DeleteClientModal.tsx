...
// RPC
import { useOauth2ClientDeleteMutation } from "src/services/rpcOAuth2";
...
const DeleteOAuth2ClientModal = (props: PropsToDelete) => {
  const dispatch = useAppDispatch();

  const [deleteClients] = useOauth2ClientDeleteMutation();
  ...
  const fields = [
    {
      ...
      pfComponent: (
        <Content component={ContentVariants.p}>
          Are you sure you want to delete the selected OAuth2 client(s)?
        </Content>
      ),
    },
    {
  ...
  const deleteOAuth2Clients = () => {
    ...
    deleteClients(elementsToDelete).then((response) => {
      if ("data" in response) {
        ...
        if (result) {
          if ("error" in result.results[0] && result.results[0].error) {
            ...
            dispatch(
              addAlert({
                name: "remove-oauth2clients-success",
  ...
  const modalActionsDelete: JSX.Element[] = [
    <Button
      ...
      key="delete-oauth2clients"
      variant="danger"
      onClick={deleteOAuth2Clients}
      form="delete-oauth2clients-modal"
    ...
    <Button
      ...
      key="cancel-delete-oauth2clients"
      variant="link"
  ...
  return (
    <>
      <ModalWithFormLayout
        dataCy="delete-oauth2clients-modal"
        variantType="medium"
        modalPosition="top"
        offPosition="76px"
        title="Remove OAuth2 clients"
        formId="remove-oauth2clients-modal"
        fields={fields}
        show={props.show}
      ...
      {isModalErrorOpen && (
        <ErrorModal
          dataCy="delete-oauth2clients-modal-error"
          title={errorTitle}
...
export default DeleteOAuth2ClientModal;
