...
// RPC
import { useOauth2ScopeDeleteMutation } from "src/services/rpcOAuth2";
...
const DeleteOAuth2ScopeModal = (props: PropsToDelete) => {
  const dispatch = useAppDispatch();

  const [deleteScopes] = useOauth2ScopeDeleteMutation();
  ...
  const fields = [
    {
      ...
      pfComponent: (
        <Content component={ContentVariants.p}>
          Are you sure you want to delete the selected OAuth2 scope(s)?
        </Content>
      ),
    },
    {
  ...
  const deleteOAuth2Scopes = () => {
    ...
    deleteScopes(elementsToDelete).then((response) => {
      if ("data" in response) {
        ...
        if (result) {
          if ("error" in result.results[0] && result.results[0].error) {
            ...
            dispatch(
              addAlert({
                name: "remove-oauth2scopes-success",
  ...
  const modalActionsDelete: JSX.Element[] = [
    <Button
      ...
      key="delete-oauth2scopes"
      variant="danger"
      onClick={deleteOAuth2Scopes}
      form="delete-oauth2scopes-modal"
    ...
    <Button
      ...
      key="cancel-delete-oauth2scopes"
      variant="link"
  ...
  return (
    <>
      <ModalWithFormLayout
        dataCy="delete-oauth2scopes-modal"
        variantType="medium"
        modalPosition="top"
        offPosition="76px"
        title="Remove OAuth2 scopes"
        formId="remove-oauth2scopes-modal"
        fields={fields}
        show={props.show}
      ...
      {isModalErrorOpen && (
        <ErrorModal
          dataCy="delete-oauth2scopes-modal-error"
          title={errorTitle}
...
export default DeleteOAuth2ScopeModal;
