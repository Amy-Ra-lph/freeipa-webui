...
import { useDeleteSelfServicesMutation } from "src/services/rpcSelfService";

interface DeleteSelfServiceModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const DeleteSelfServiceModal = (props: DeleteSelfServiceModalProps) => {
  const dispatch = useAppDispatch();

  // RPC calls
  const [executeSelfServiceDelCommand] = useDeleteSelfServicesMutation();
    ...
    },
    {
      ...
      pfComponent: (
        <DeletedElementsTable
          ...
          elementType="Self service permission"
          idAttr="aciname"
        />
      ),
    },
  ...
  // Delete handler
  const onDeleteSelfServices = () => {
    setBtnSpinning(true);

    executeSelfServiceDelCommand(props.elementsToDelete).then((response) => {
      if ("data" in response) {
        ...
        if (result) {
          ...
          if (errorResults.length > 0) {
            ...
            dispatch(
              addAlert({
                name: "remove-self-service-success",
  ...
  const modalActions: JSX.Element[] = [
    <Button
      key="delete-self-service"
      variant="danger"
      onClick={onDeleteSelfServices}
      form="delete-self-service-modal"
    ...
    <Button
      key="cancel-delete-self-service"
      variant="link"
  ...
  return (
    <>
      <ModalWithFormLayout
        dataCy="delete-self-service-modal"
        variantType="medium"
        modalPosition="top"
        offPosition="76px"
        title="Remove self service permissions"
        formId="delete-self-service-modal"
        fields={fields}
        show={props.isOpen}
      ...
      {isModalErrorOpen && (
        <ErrorModal
          dataCy="delete-self-service-modal-error"
          title={errorTitle}
...
export default DeleteSelfServiceModal;
