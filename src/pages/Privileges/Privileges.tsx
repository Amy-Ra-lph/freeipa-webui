...
const Privileges = () => {
  ...
  const { browserTitle } = useUpdateRoute({ pathname: "privileges" });
  ...
  const privilegesDataResponse = useGettingPrivilegesQuery({
    searchValue: "",
    sizeLimit: 0,
  ...
  const [searchPrivileges, searchResult] =
    useSearchPrivilegesEntriesMutation({});
  ...
  const selectablePrivilegesTable =
    elementsList.filter(isPrivilegeSelectable);
  ...
  const updateSelectedPrivileges = (
    ...
    if (isSelected) {
      ...
      for (let i = 0; i < privileges.length; i++) {
        if (
          selectedPrivileges.find((s) => s.cn[0] === privileges[i].cn[0])
        ) {
          continue;
        }
