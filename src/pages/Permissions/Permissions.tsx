...
const Permissions = () => {
  ...
  const { browserTitle } = useUpdateRoute({ pathname: "permissions" });
  ...
  const permissionsDataResponse = useGettingPermissionsQuery({
    searchValue: "",
    sizeLimit: 0,
  ...
  const [searchPermissions, searchResult] =
    useSearchPermissionsEntriesMutation({});
  const [isSearchActive, setIsSearchActive] = useState(false);
  const [searchData, setSearchData] =
    useState<SearchDataResultType<Permission> | null>(null);

  // Derive permissionsList and totalCount from query response or search results
        ...
        if (searchError) {
          ...
          dispatch(
            addAlert({
              ...
              title: error || "Error when searching for permissions",
              variant: "danger",
            })
          );
          setIsSearchActive(false);
          ...
          const permissionsListResult = result.data?.result.results || [];
          const permissionsListSize = result.data?.result.count || 0;
  ...
  const selectablePermissionsTable =
    elementsList.filter(isPermissionSelectable);
  ...
  const updateSelectedPermissions = (
    ...
    if (isSelected) {
      newSelectedPermissions = JSON.parse(
        JSON.stringify(selectedPermissions)
      );
      for (let i = 0; i < permissions.length; i++) {
        if (
          selectedPermissions.find(
            (s) => s.cn[0] === permissions[i].cn[0]
          )
        ) {
          continue;
        }
  ...
  const paginationData = {
    ...
    updateShownElementsList: (permissions: Permission[]) => {
      if (isSearchActive) {
        setSearchData((prev) =>
          prev
            ? { ...prev, elementsList: permissions }
            : { elementsList: permissions, totalCount: 0 }
        );
      }
    },
    totalCount,
  ...
  const keyNames = ["cn", "ipapermright", "ipapermbindruletype", "type"];
